...
import {
    Box, Button, VStack, Text, Flex, CloseButton, HStack, Icon,
    Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, useMediaQuery
            ...
            {!isFlipped && (
                <>
                    <VStack
                        ...
                        alignItems="left"
                    >
                        <Text fontSize={{ base: "sm", md: "md", lg: "lg" }} fontFamily="primaryFont" fontWeight="medium" color="white">{slide.name}</Text>
                        <Text fontSize={{ base: "xs", md: "sm", lg: "md" }} fontFamily="bannerFont" color="white">{slide.description}</Text>
                    </VStack>
                ...
                <>
                    ...
                    >
                        ...
                        justifyContent={{"base":"center","md":"flex-start"}}>
                            <HStack spacing={{ "base": "2em", "md": "2em", "lg": "1.5em" }} onClick={() => isLargerThanBase ? null : handleModal(slide.extendedDescription)}>
                                <Icon as={InfoIcon} />
                                <Text fontWeight="bold">Extended Description</Text>
                                {isLargerThanBase && <Text>{slide.extendedDescription}</Text>}
                            </HStack>
                            <HStack spacing={{ "base": "2em", "md": "2em", "lg": "1.5em" }} onClick={() => isLargerThanBase ? null : handleModal(slide.challenges)}>
                                <Icon as={SpinnerIcon} />
                                <Text fontWeight="bold">Challenges Faced</Text>
                                {isLargerThanBase && <Text>{slide.challenges}</Text>}
                            </HStack>
                            <HStack spacing={{ "base": "2em", "md": "2em", "lg": "1.5em" }} onClick={() => isLargerThanBase ? null : handleModal(slide.technologies)}>
                                <Icon as={SettingsIcon} />
                                <Text fontWeight="bold">Main Technologies</Text>
                                {isLargerThanBase && <Text>{slide.technologies}</Text>}
                            </HStack>
                            <HStack spacing={{ "base": "2em", "md": "2em", "lg": "1.5em" }} onClick={() => isLargerThanBase ? null : handleModal(slide.time)}>
                                <Icon as={TimeIcon} />
                    ...
                    <Box position="absolute" top="2%" right="2%" bg="white" borderRadius="50%" >
                        <CloseButton size={{"base":"md","md":"md","lg":"lg"}} color="black" onClick={handleFlip} />
                    </Box>
                </>
            )}
        </Box>
...
function NavigationButton({ direction, handleClick }) {
    ...
    return (
        <Button {...buttonStyle} onClick={handleClick} _hover={{ backgroundColor: "transparent" }}>
